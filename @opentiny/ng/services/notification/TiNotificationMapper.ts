...
export class TiNotificationMapper {
  private _mapper = new Map<string, TiNotificationMapperItem>();

  private static readonly KEY: string = 'ti-notification';

  setItem(target: any): void {
    const item: TiNotificationMapperItem = { target };
    this._mapper.set(TiNotificationMapper.KEY, item);
  }

  getItem<T>(): T | null {
    if (this._mapper.has(TiNotificationMapper.KEY)) {
      return this._mapper.get(TiNotificationMapper.KEY)!.target as T
    }
    return;
  }
}