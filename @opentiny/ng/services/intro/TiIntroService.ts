...
import { TiKeymap, TiLog, Util } from '../../utils/Util';
import { Position, TiPositionEventType } from '../../utils/Position';
...
export class TiIntroService {
    ...
    private static scrollToViewport(element: any): void {
        ...
        if (layout.top < 0) {
            ...
            // 下边界溢出屏幕
            window.scrollTo({top: scrollTop + layout.bottom + 20 - document.documentElement.clientHeight});
        }
    }
    ...
    public create(config: TiIntroConfig): TiIntroRef {
        ...
        const wantStep: (number: number) => void = (num: number): void => {
            ...
            if ((<any>window).getConsoleContext && !isAddConsoleDataOnChange) {
                consoleDataService = (<any>window).getConsoleContext()
                    ?.get({ name: 'safearea' });
                // 头部高度变化会触发此事件
