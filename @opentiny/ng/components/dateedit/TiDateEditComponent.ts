...
export class TiDateEditComponent extends TiFormComponent {
    ...
    private setPlacehoderText(): void {
        ...
        } else if (this.format === 'mediumDate') { // 显示格式为"mediumDate"
            this.inputFormat = TiLocale.getLocaleWords().tiLocaleKey === TiLocale.ZH_CN ? 'yyyy年M月dd日' : 'MM/d/yyyy';
        } else {
    ...
    private getValidModel(): void {
        ...
        if (/[年月日]/.test(this.inputValue) && !dateArr[dateArr.length - 1].length) {
            dateArr.pop();
        }
