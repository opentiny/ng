...
import {
    ...
    EventEmitter, Input, Output, Renderer2, SimpleChanges, ViewChild
} from '@angular/core';
...
export class TiSpinnerComponent extends TiFormComponent {
    ...
    private setInputTip(): void {
        ...
        if (this.max !== Math.pow(2, 53) && this.min !== -Math.pow(2, 53)) {
            this.inputTip = Util.formatEntry(TiLocale.getLocaleWords().tiValid.errorMsg.rangeValue, [this.min, this.max]);

            return;
        }

        if (this.min !== -Math.pow(2, 53)) {
            this.inputTip =  Util.formatEntry(TiLocale.getLocaleWords().tiValid.errorMsg.minValue, [this.min]);

            return;
        }

        if (this.max !== Math.pow(2, 53)) {
            this.inputTip = Util.formatEntry(TiLocale.getLocaleWords().tiValid.errorMsg.maxValue, [this.max]);
