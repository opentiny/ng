 ...
 *
 */
import {ChangeDetectionStrategy, ChangeDetectorRef, Component, ContentChild, ElementRef, EventEmitter,
    Input, IterableChanges, IterableDiffer, IterableDiffers, KeyValueChanges,
    KeyValueDiffer, KeyValueDiffers, Output, Renderer2, TemplateRef, ViewChild } from '@angular/core';
...
export class TiActionmenuComponent extends TiFormComponent {  // 本应继承自BaseComp，但是想要A标签焦点功能
        ...
        if (this.items) {
            if (!this.itemsDiffer) {
                this.itemsDiffer = this.iterableDiffers.find(this.items)
                    .create((index: number, item: TiActionmenuItem) => {
    ...
    private findfirstChildrenAndSetDisplay(): number {
      ...
      for (let i: number = 0; i < this.items.length; i++) {
        ...
        if (this.dividingElems[i]) { // 动态修改item，第二次进入时需要重置分割线可见
    ...
    private getAndSetOutShowNum(myFirstChildrenIndex: number): number {
      ...
      for (let i: number = this.items.length - 1; i >= 0; i--) {
        ...
        } else if (!TiActionmenuComponent.isEqualOffsetTop(this.menuElem, this.itemElems[0])) { // 菜单没有在第一行，删除本项
          this.setDisplay(this.itemElems[i], false);
          if (this.dividingElems[i]) {
            this.setDisplay(this.dividingElems[i], false);
          }
  ...
  public itemElemsEqual(): boolean {
    if (this.itemElems?.length !== TiActionmenuComponent.lastItemElems?.length || this.nativeElement.parentNode.tagName !=='TD'
    || this.maxWidth !== TiActionmenuComponent.lastMaxWidth || this.maxShowNum !== TiActionmenuComponent.lastMaxShowNum) {
      return false;
    }
