 ...
 *
 */
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, ContentChild, ElementRef, EventEmitter, Input,
    IterableChanges, IterableDiffer, IterableDiffers, Output, QueryList, Renderer2, SimpleChanges, TemplateRef,
    ...
    */
    ...
    ngOnInit(): void {
        ...
        this.dataDiffer = this.iterableDiffers.find(this.data)
                .create((index: number, item: TiTreeNode) => {
                    return item.checked;
                });
        // 内部使用的数据，用于记录用户的操作改变
