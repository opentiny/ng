...
export class TiTreeUtil {
     ...
     */
    public static getSelectedData(data: Array<TiTreeNode>, onlySelectLeaf: boolean, multiple: boolean, checkRelation?: boolean): Array<TiTreeNode> {
        // 每次遍历之前需要清空当前选中项列表
     ...
     * @param checkRelation 父子节点选中状态是否关联
     */
    public static querySelectedNode(data: Array<TiTreeNode>, onlySelectLeaf: boolean, multiple: boolean, checkRelation?: boolean): void {
        let tempNode: any;
        ...
        for (let i: number = 0; i < data.length; i++) {
            ...
            if (multiple && !relation && !TiTreeUtil.isLeaf(tempNode)) {
                ...
                TiTreeUtil.querySelectedNode(tempNode.children, onlySelectLeaf, multiple, relation);
     ...
     * @param multiple 多选
     */
    public static dropApply(event: TiTreeDragNode, data: Array<TiTreeNode>, multiple?: boolean): void {
        if (!Array.isArray(data)) {
            return;
        }
