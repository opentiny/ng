...
export class TiDatetimeRangeComponent extends TiDateBaseComponent {
     ...
     * 时区选择下拉建议项
     */
    public timeZoneOptions: Array<any> = [
        { label: TiLocale.getLocaleWords().tiDate.utcZone},
        { label: TiLocale.getLocaleWords().tiDate.localZone}
    ];
    /**
     * @ignore
     * 时区选择当前选中项
    ...
    public onOkClick(): void {
        ...
        // 中间空格误删
        const beginValue: Date = this.getLocalDate(`${beginDateStr} ${TiDateUtil.addColon(this.beginTime['value'])}`);
        const endDateStr: string = TiDateUtil.getDateStr(this.datePanel['value']['end']);
        // 中间空格误删
        const endValue: Date = this.getLocalDate(`${endDateStr} ${TiDateUtil.addColon(this.endTime['value'], true)}`);
    ...
    public formatValue(): void {
        if (this.model === null || (this.model.begin === null && this.model.end === null)) {
            ...
            const begin: string = TiLocaleFormat.formatDateTime(this.getUtcDate(this.model.begin), format);
            const end: string = TiLocaleFormat.formatDateTime(this.getUtcDate(this.model.end), format);
    ...
    private setDatePanelOptions(): void {
        ...
        if (this.model === null || this.model['begin'] === null && this.model['end'] === null) {
            ...
            value = {
                begin: this.getUtcDate(this.model.begin),
                end: this.getUtcDate(this.model.end)
            };
        }
        ...
        this.datePanel = {
            ...
            picker: 'day',
            select: (): void => {
                const nowTime: string =  TiLocaleFormat.formatTime(this.getUtcDate(new Date()), this.format.time);
                ...
                if (this.focusedPosition === 'begin') {
                    ...
                    this.dayClick.emit({
                        value: this.getLocalDate(`${beginValue}`),
                        focusedPosition: this.focusedPosition,
                        beginValue: this.getLocalDate(`${this.datePanel.value.begin}`),
                        endValue: this.getLocalDate(`${this.datePanel.value.end}`)
                    });
                    ...
                    this.dayClick.emit({
                        value: this.getLocalDate(`${endValue}`),
                        focusedPosition: this.focusedPosition,
                        beginValue: this.getLocalDate(`${this.datePanel.value.begin}`),
                        endValue: this.getLocalDate(`${this.datePanel.value.end}`)
                    });
    ...
    public timeBlur(pos: string, val: string): void {
        ...
        if (pos === 'begin' && TiDateUtil.isDate(this.datePanel.value.begin)) {
            ...
            const beginDateVal: Date = new Date(`${TiDateUtil.getDateStr(this.datePanel.value.begin)} ${TiDateUtil.addColon(this.beginTime.value)}`);
            ...
            if (String(beginDate) === 'Invalid Date' || String(dateVal) === 'Invalid Date' || timeArr.length > 3 || isNotInteger || (!this.isBeginSmallerThanEnd() && TiDateUtil.isDate(this.datePanel.value.end)) || isSmallerThanMin) {
            ...
            const endDateVal: Date = new Date(`${TiDateUtil.getDateStr(this.datePanel.value.end)} ${TiDateUtil.addColon(this.endTime.value)}`);
            ...
            if (String(endDate) === 'Invalid Date' || String(dateVal) === 'Invalid Date' || timeArr.length > 3 || isNotInteger || (!this.isBeginSmallerThanEnd() && TiDateUtil.isDate(this.datePanel.value.begin)) || isBiggerThanMax) {
    ...
    public dateEditBlur(val: any, pos: string): void {
        ...
        if (TiDateUtil.isBetweenMaxAndmin(val, this.min, this.max)) {
            if (pos === 'begin') {
                ...
                if (this.beginTimeDisabled) {
                    this.beginTime['value'] = TiLocaleFormat.formatTime(this.getUtcDate(new Date()), this.format.time);
                    this.beginTimeDisabled = false;
                }

            } else {
                if (this.endTimeDisabled) {
                    this.endTime['value'] = TiLocaleFormat.formatTime(this.getUtcDate(new Date()), this.format.time);
                    this.endTimeDisabled = false;
                }
    ...
    private setBeginTimeValue(): void {
        ...
        this.beginTime['value'] = TiLocaleFormat.formatTime(this.getUtcDate(this.model.begin), this.format.time);
    }
    ...
    public setBeginTimeMaxValue(): void {
        ...
        this.beginTime['max'] = ((value instanceof Object) && TiDateUtil.isDateEqual(this.max, value['begin'])) ?
                                TiLocaleFormat.formatTime(this.max, this.format.time) : maxFormatTime;
    }
    ...
    public setBeginTimeMinValue(): void {
        ...
        this.beginTime['min'] = ((value instanceof Object) && TiDateUtil.isDateEqual(this.min, value['begin'])) ?
                                TiLocaleFormat.formatTime(this.min, this.format.time) : minFormatTime;
    }
    ...
    private setEndTimeValue(): void {
        ...
        this.endTime['value'] = TiLocaleFormat.formatTime(this.getUtcDate(this.model.end), this.format.time);
     ...
     */
     public customizeTimeClickFn(val: TiDateValue): void {
        this.model = {
            begin: this.getLocalDate(`${val.begin}`),
            end: this.getLocalDate(`${val.end}`)
        };
    ...
    private isBeginSmallerThanEnd(): boolean {
        ...
        const end: Date = new Date(
            `${TiDateUtil.getDateStr(this.datePanel.value.end)} ${TiDateUtil.addColon(this.endTime['value'], true)}`);

        return this.isAllowBeginEqualEnd ? begin.getTime() <= end.getTime() : begin.getTime() < end.getTime();
    }

    // 根据时区获取当前选中本地时间
    private getLocalDate(dateString: string): Date {
        if (dateString === 'null') {
            return null;
        }

        // 将UTC时间转化为本地时间
        return this.timeZoneable && this.selectedOption === 'UTC/GMT' ?
               new Date(`${dateString} UTC`) : new Date(`${dateString}`);
    }

    // 根据时区获取当前选中UTC时间（此处的UTC时间指的并不是实际意义上的UTC时间，而是根据UTC时间转化为对应的本地时间呈现给用户）
    private getUtcDate(date: Date): Date {
        ...
        return this.timeZoneable && this.selectedOption === 'UTC/GMT' ?
               new Date(date.getTime() + date.getTimezoneOffset() * 60 * 1000) : date;
    }
}
