...
export class TiTimeComponent extends TiFormComponent {
   ...
   @ViewChild('input', { static: true }) inputEle: ElementRef;
    ...
    public timeBlur(val: string): void {
        ...
        const datetime: Date = new Date(`2022/01/28 ${TiDateUtil.addColon(val)}`);
    ...
    private getScrollData() {
        if (Util.isEmptyString(this.inputValue)) {
            ...
            const timeArr: Array<any> = this.inputValue.split(TiDateUtil.COLON_REGEXP);
    ...
    private setDisableData(isChange?: boolean): boolean {
        ...
        } else if (Number(timeArr[0]) === Number(minArr[0])) { // 最小小时情况,需考虑分钟禁用
            this.minuteOptions.forEach((minuteItem: any) => {
                minuteItem.disabled = Number(minuteItem.label) < Number(minArr[1]) ;
            });
        } else if (Number(timeArr[0]) === Number(maxArr[0])) { // 最大小时情况,需考虑分钟禁用
            this.minuteOptions.forEach((minuteItem: any) => {
        ...
        if (Number(timeArr[1]) && Number(minArr[0]) === Number(maxArr[0]) && Number(minArr[1]) === Number(maxArr[1])) { // 最大小时分最小小时分相等情况,需考虑分钟禁用
        ...
        } else if (Number(timeArr[0]) === Number(minArr[0]) && Number(timeArr[1]) === Number(minArr[1])) { // 最小时分情况,需考虑秒数禁用
            this.secondOptions.forEach((secondItem: any) => {
