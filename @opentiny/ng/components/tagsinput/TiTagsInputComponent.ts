...
export class TiTagsInputComponent extends TiWholeComponent {
      ...
      onInputChange(inputValue: string): void {
            ...
            if (this.valueKey) {
                  this.selectDrop.searchResult.unshift({ id: inputValue,
                        [this.labelKey]: inputValue, [this.valueKey]: inputValue, isInput: true });
            } else {
                  this.selectDrop.searchResult.unshift({ id: inputValue, [this.labelKey]: inputValue, isInput: true });
            }
      }
      ...
      private addTagToSelected(inputValue: string): void {
            ...
            // 有labelkey和valueKey时，需要设置对应键值
            const newTag: any = index === -1 ?
                        (this.valueKey ?  { id: inputValue, [this.labelKey]: inputValue , [this.valueKey]: inputValue } :
                        { id: inputValue, [this.labelKey]: inputValue }) : this.suggestions[index];
