  ...
  // TODO: add explicit constructor
    ...
    ngOnModelChange(): void {
        ...
        if (this.model?.length > 0 && this.valueKey) {
            this._modelWhole = this.items.filter((item) => this.model.includes(this.valueFn(item)));
            if (this._modelWhole[0].children) {
                const nextModelWhole: TiCascaderItem
                    = this._modelWhole[0].children.filter((item) => this.model.includes(this.valueFn(item)))[0];
                if (nextModelWhole) {
                    this._modelWhole.push(nextModelWhole);
                    if (nextModelWhole.children) {
                        const nextModelWhole1: TiCascaderItem =
                            nextModelWhole.children.filter((item) => this.model.includes(this.valueFn(item)))[0];
    ...
    public onSearch(event: string): void {
        ...
        this.items.forEach((items1: TiCascaderItem, index1: number): void => {
            ...
            // 在该项本身搜索
            if ((!this.onlySelectLeaf || !items1.children || items1.children?.length === 0)
            ...
            if (items1.children?.length > 0) {
                items1.children.forEach((items2: TiCascaderItem, index2: number): void => {
                    ...
                    if (!items2.children || items2.children?.length === 0 || !this.onlySelectLeaf) {
                        if (lowercaseItems1Label.includes(lowercaseSearchword)
                        ...
                        || `${lowercaseItems1Label}/${lowercaseItems2Label}`.includes(lowercaseSearchword)) {
                            this.wideDroplistOptions.push({
                                ...
                                labels: [items1[this.labelKey], items2[this.labelKey]]
                            });
                            if (!flag && this.modelWhole?.length  === 2
                        ...
                        items2.children.forEach((items3: TiCascaderItem, index3: number): void => {
                            ...
                            || `${lowercaseItems1Label}/${lowercaseItems2Label}/${lowercaseItems3Label}`.includes(lowercaseSearchword)) {
                                    this.wideDroplistOptions.push({
                                        ...
                                        labels: [items1[this.labelKey], items2[this.labelKey], items3[this.labelKey]]
                                    });
                                    if (!flag && this.modelWhole?.length  === 3
                                        && this.modelWhole[0] === items1
                                        && this.modelWhole[1] === items2
    ...
    private isSame(option1: TiCascaderItem, option2: TiCascaderItem): boolean {
        ...
        return option1 === option2 || (option1[this.labelKey] !== undefined && option2[this.labelKey] !== undefined
               && option1[this.labelKey] === option2[this.labelKey]);
    }
    /**
