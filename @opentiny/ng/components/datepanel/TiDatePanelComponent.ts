...
export class TiDatePanelComponent extends TiBaseComponent {
    ...
    private changePicker(changes): void {
    ...
    private changeScope(changes): void {
    ...
    private changesSelectTime(changes): void {
    ...
    private changeBeginTime(changes): void {
    ...
    private changeEndTime(changes): void {
    ...
    private changeEndValue(changes): void {
        if (TiDateUtil.isDate(this.value.end) && changes['focusedPosition'].currentValue === "end") {
            ...
            this.date = {
                year: (this.picker === 'onlyYearMonth' && this.date.year < this.value.end.getFullYear())
                    || (this.picker === 'quarter' && this.date.year < this.value.end.getFullYear())
                    ? this.value.end.getFullYear() - 1 :
                    this.picker === 'onlyYear' && isHideEndvalue ? this.value.end.getFullYear() - 10 :
                        this.value.end.getFullYear(),
                // 同年同月场景下，点击切换焦点面板不需要切换
                month: this.value.end.getMonth() === this.value.begin?.getMonth() &&
                    this.value.end.getFullYear() === this.value.begin?.getFullYear() ?
                    this.value.end.getMonth() + 1 : this.value.end.getMonth(),
                day: this.value.end.getDate()
            };
        }
    }
    private changesFocusedPosition(changes): void {
    ...
    private changesTimeFormat(changes): void {
    ...
    */
    ...
    private setTimePanel(): void {
        ...
        if (this.timeFormat?.indexOf('m') === -1) {
            this.onlyHour = true;
        } else if (this.timeFormat?.indexOf('s') === -1) { // 仅时分
