...
import { Component, ContentChild, ElementRef, EventEmitter, HostListener,
    Input, NgZone, Output, Renderer2, SimpleChange, SimpleChanges, TemplateRef, ViewChild, ChangeDetectorRef, ChangeDetectionStrategy} from '@angular/core';
...
export class TiListComponent extends TiFormComponent {
    ...
    ngAfterViewInit(): void {
        ...
        if (this.scrollToBottom.observers.length > 0 && this.virtualScrollViewport) {
            this.scrollEventSub = this.virtualScrollViewport.elementScrolled()
    ...
    private selectAllOptions(): void {
        ...
        if (this.selectAllState === false || this.selectAllState === null) {
            listOptions.forEach((item: any) => {
                if (this.getIndex(item, this.model) === -1 && this.isSelectable(item)) { // 不在当前选中项且为可选数据项（排除禁用项和组名项）
            ...
            if (this.hasSelectedinList()) {
                ...
                if (this.multiple && this.selectAll) {
                    ...
                    if (this.options[0]?.children) {
                        ...
                        this.hoverOption = this.options.find(
                            (option: any) => !this.isDisabledFn(option)
                        );
                    }
                }
            }
    ...
    private getHoverOptionInModel(): any {
        return this.multiple
            ? (this.options[this.getIndex(this.model.find((item: any) => !this.isDisabledFn(item)), this.options)]
    ...
    private isEqual(modelOption: any, option: any): boolean {
        ...
        return modelOption === option || (modelOption[this.labelKey] !== undefined && option[this.labelKey] !== undefined
