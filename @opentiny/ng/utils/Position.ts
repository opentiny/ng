...
export class Position {
    ...
    public static setPosition(options: {
        ...
        // 分别获取宿主和待定位元素当前位置
        const curLayout: TiPositionLayout = Position.getLayout(options.hostEle, options.targetEle, options.hostEleX, options.referElem);
    ...
    private static getLayout(hostEle: any, targetEle: any, horizonHostEle: any, referElem?: any): TiPositionLayout {
        ...
        if (Util.isElement(referElem)) {
            const hostOffsetLayout: any = referElem === hostEle ? {
                left: 0,
                top: 0
            } : {
    ...
    private static setLayout(ele: any, position: TiLayoutParams, layoutParam: TiPositionLayout, hOffset: number): void {
        if (layoutParam.hostLayout.fixedAncestor && !layoutParam.hostOffsetLayout) { // fixed定位情况下，滚动条对其不受影响，因此此处需要减掉滚动条的位置影响
    ...
    private static getHorizionParam(curLayout: TiPositionLayout, posArr: Array<string>, space: number, hasOffsetFix: boolean): number {
        const { hostOffsetLayout, hostLayout, targetLayout }:
            { hostOffsetLayout?: any, hostLayout: TiHostLayout, targetLayout: TiEleLayout } = curLayout;
    ...
    TiVerticalParams {
        ...
        if (posArr[0] === 'top') {
            ...
            const top: number = (avilableHeight >= targetLayout.height) ?
                (hostTop - targetLayout.height - hostSpace) :
        ...
        const topTemp: number = (avilableHeight >= targetLayout.height) ? hostTop - (targetLayout.height - hostLayout.height) / 2 :
        hostTop - avilableLayout.top + browserSpace;
