...
export class TiDateUtil {
    ...
    public static transFormQuarterToDate(year: number, val: string, isEnd?: boolean): any {
        if (isEnd) {
            switch (val) {
                case 'Q1': return new Date(year, 2, 31, 23, 59, 59, 999);
                case 'Q2': return new Date(year, 5, 30, 23, 59, 59, 999);
                case 'Q3': return new Date(year, 8, 30, 23, 59, 59, 999);
                case 'Q4': return new Date(year, 11, 31, 23, 59, 59, 999);
                default: return undefined;
            }
        } else {
            switch (val) {
                case 'Q1': return new Date(year, 0, 1, 0, 0, 0);
                case 'Q2': return new Date(year, 3, 1, 0, 0, 0);
                case 'Q3': return new Date(year, 6, 1, 0, 0, 0);
                case 'Q4': return new Date(year, 9, 1, 0, 0, 0);
                default: return undefined;
            }
        }
    }
            ...
            switch (picker) {
                ...
                case 'month':
                    ...
                    return new Date(year, month,
                        new Date(value.getFullYear(), month + 1, 0).getDate(), 23, 59, 59, 999);
                default:
        ...
        if (isEnd) {
            switch (dateTimePicker) {
                ...
                case 'onlyHoursMinutes':
                    // 纯时分格式下，结束具体日期时间到59秒
                    return value = new Date(year, month, date, hours, minutes, 59, 999);
                default:
                    // 正常情况下，结束时间值具体到秒
            ...
            switch (dateTimePicker) {
                ...
                case 'onlyHoursMinutes':
                    // 纯时分格式下，结束具体日期时间到分
                    return value = new Date(year, month, date, hours, minutes);
                default:
                    // 正常情况下，结束时间值具体到秒
