...
export class CheckgroupTreeComponent {
  ...
  femalesCheckeds: Array<any> = this.females.filter(
    (x: any) => (x as any).checked
  );
  /*方案一end************************************************ */
  /*方案二start************************************************ */
  // 数据列表
  ...
  malesCheckeds2: Array<any> = this.males2.filter(
    (x: any) => (x as any).checked
  ); // 不用as any的话，ts编译报错，因为上面定义的选项数据，有的项目没有chekced属性。
  femalesCheckeds2: Array<any> = this.females2.filter(
    (x: any) => (x as any).checked
  );
  /*方案二end************************************************* */
  /*方案三start************************************************ */
  ...
  rootCheckeds3: Array<any> = this.dataArray3.filter(
    (x: any) => (x as any).checked
  ); // 不用as any的话，ts编译报错，因为上面定义的选项数据，有的项目没有chekced属性。
  malesCheckeds3: Array<any> = this.males3.filter(
    (x: any) => (x as any).checked
  );
  femalesCheckeds3: Array<any> = this.females3.filter(
    (x: any) => (x as any).checked
  );
  /*方案三end************************************************* */
  myitems: Array<any> = [
