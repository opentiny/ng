...
export class TableStorageServeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let storageState =
      typeof window !== 'undefined'
        ? window.localStorage.getItem('this.storageId')
        : undefined;
    if (!storageState) {
      this.getCurrentPageData(this.currentPage, this.pageSize.size).then(
  ...
  stateUpdate(tiTable: TiTableComponent): void {
    ...
    ).then((data: Array<TiTableRowData>) => {
  ...
  private getCurrentPageData(
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        for (
          let j: number = (currentPage - 1) * itemsPerPage;
          j < currentPage * itemsPerPage;
          j++
        ) {
