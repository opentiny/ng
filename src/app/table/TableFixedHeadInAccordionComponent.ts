...
@Component({
  ...
  templateUrl: './table-fixed-head-in-accordion.html',
})
export class TableFixedHeadInAccordionComponent implements OnInit {
  elementId: string = 'table_test';
  displayed: Array<TiTableRowData> = []; // 表示表格实际呈现的数据（开发者默认设置为[]即可）
  srcData: TiTableSrcData;
  data: Array<TiTableRowData> = [];
  headClass: string = 'headCls';
  bodyClass: string = 'bodyCls';
