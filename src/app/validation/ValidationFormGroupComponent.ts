...
export class ValidationFormGroupComponent {
  ...
  resetGroup(): void {
    this.groupFormControl.reset({ emailController: 'hello tiny', requiredController: '', selectControllor: '' });
  }
  selectDisabled(): void {
  ...
  checkAsyncForm(): void {
    ...
    if (pendingStatusChangesArray.length > 0) {
      const pendingStatusChanges: Subscription = zip(
        ...pendingStatusChangesArray
      )
        .subscribe((resultArray: Array<string>) => {
          resultArray.forEach((result: string, index: number) => {
            if (result === 'INVALID') {
              if (errors === null) {
                errors = {};
              }
              errors[pendingControlsArray[index].name] =
                pendingControlsArray[index].control.errors;
            }
          });
      ...
      if (control.valueChanges) {
        return control.valueChanges.pipe(
          ...
          switchMap((value: string) =>
            CustomAsyncValidators.isRight(value, delay)
          ),
          map((isRight: boolean) => {
            return isRight
              ? {
