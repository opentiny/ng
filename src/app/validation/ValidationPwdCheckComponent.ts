...
export class ValidationPwdCheckComponent {
  ...
  passwordConfig2: TiValidationConfig = {
    ...
    passwordConfig: {
      validator: {
        ...
        message: {
          rangeSize: '长度为{0}至{1}个',
          minCharType:
            '至少包含大写字母、小写字母、数字及特殊符号`!=中的{0}种',
        },
      },
    },
  };
  ...
  constructor(fb: FormBuilder) {
    const custiomRule1: (params: any) => ValidatorFn = (
      params: any
    ): ValidatorFn => {
    ...
    const custiomRule2: (params: any) => ValidatorFn = (
      params: any
    ): ValidatorFn => {
  ...
  x(param: string): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      return control.value !== param
        ? null
        : { custom1: { notEqualValue: param, actualValue: control.value } };
    };
  }
