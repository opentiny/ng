...
export class SelectPaginBeforesearchComponent {
  ...
  onCurrentPageChange1(
    currentPage: number,
    selectComp: TiSelectComponent
  ): void {
    // 模拟后台分页
  ...
  private getdatas(pageNumber?: number, searchWord?: string): Promise<any> {
    ...
    const promise: Promise<any> = new Promise(
      (resolve: any, rejects: any): void => {
        setTimeout(() => {
          if (pageNumber === undefined && searchWord === undefined) {
            rejects(new Error('没有参数'));
          }
          if (searchWord) {
            const filteredResult: Array<any> = this.data.filter((item) =>
              item.label.toLowerCase().includes(searchWord.toLowerCase())
            );
            const slicedResult: Array<any> = filteredResult.slice(
              this.pageSize.size * (pageNumber - 1),
