...
@Component({
  ...
  templateUrl: './table-soldout.html'
})
export class TableSoldoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.checkedList.push(this.data[2]);
    this.checkedList.push(this.data[6]); // 初始选中项
  }
  ...
  private createRandomItem(id: number): TiTableRowData {
    ...
    const messageArr: Array<any> = [
      { text: '', class: 'ti3-table-row-message-soldout', iconName: '' },
      { text: 'New', class: 'ti3-table-row-message-new', iconName: '' },
      {
        text: '',
        class: 'ti3-table-row-message-recommended',
        iconName: 'discount-sup'
      }
    ];
    const message: any = messageArr[((id + 3) * 19) % 3];
