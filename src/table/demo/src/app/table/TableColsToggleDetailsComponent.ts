...
@Component({
  ...
  templateUrl: './table-cols-toggle-details.html'
})
export class TableColsToggleDetailsComponent implements OnInit {
  displayed: Array<TiTableRowData> = []; // 表示表格实际呈现的数据（开发者默认设置为[]即可）
  srcData: TiTableSrcData;
  private data: Array<TiTableRowData> = [];
  columns: Array<TiTableColumns> = [
    {
      title: '' // title为空字符串时，该列信息不在控制列隐藏/显示的下拉中出现，对于操作列适用于改场景
    },
    {
      title: 'first name',
      width: '20%',
